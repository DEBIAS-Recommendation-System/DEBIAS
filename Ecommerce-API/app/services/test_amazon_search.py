...
def test_text_searches():
    ...
    for query in queries:
        ...
        results = qdrant_service.search(
            query_text=query, limit=5, collection_name="amazon_products"
        )
        ...
        if results:
            for i, result in enumerate(results, 1):
                ...
                print(
                    f"   Category: {result['payload']['category']:30s} | Price: ${result['payload']['price']:7.2f} | Score: {result['score']:.4f}"
                )
        else:
...
def test_category_filtering():
    ...
    results = qdrant_service.search(
        query_text="shoes", limit=3, collection_name="amazon_products"
    )

    for i, result in enumerate(results, 1):
        print(
            f"{i}. {result['payload']['title'][:50]} - {result['payload']['category']}"
        )
...
def test_price_range():
    ...
    results = qdrant_service.search(
        query_text="product", limit=10, collection_name="amazon_products"
    )

    for i, result in enumerate(results, 1):
        print(
            f"{i}. {result['payload']['title'][:45]:45s} | ${result['payload']['price']:8.2f}"
        )
