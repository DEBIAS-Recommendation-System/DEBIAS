...
if common_ids:
    ...
    for result in image_results:
        if result['id'] in common_ids:
            ...
            print(f'     Brand: {result["payload"]["brand"]} | Price: ${result["payload"]["price"]:.2f}')
...
for i, result in enumerate(filtered_results, 1):
    ...
    print(f'     Category: {result["payload"]["category"]} | Price: ${result["payload"]["price"]:.2f}')
...
for i, result in enumerate(image_filtered, 1):
    print(f'  {i}. {result["payload"]["title"][:60]}')
    print(f'     Brand: {result["payload"]["brand"]} | Similarity: {result["score"]:.4f}')
...
for query in cross_modal_queries:
    ...
    if results:
        print(f'   Top match: {results[0]["payload"]["title"][:50]}... (score: {results[0]["score"]:.4f})')
