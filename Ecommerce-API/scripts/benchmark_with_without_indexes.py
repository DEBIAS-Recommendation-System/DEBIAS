...
def benchmark_with_without_indexes(m_value: int, iterations: int = 50):
    ...
    client.create_collection(
        ...
        hnsw_config=HnswConfigDiff(
            m=m_value, ef_construct=100, full_scan_threshold=10000
        ),
    )
    ...
    p95_no_idx = (
        statistics.quantiles(latencies_no_idx, n=20)[18]
        if len(latencies_no_idx) >= 20
        else max(latencies_no_idx)
    )
    ...
    p95_with_idx = (
        statistics.quantiles(latencies_with_idx, n=20)[18]
        if len(latencies_with_idx) >= 20
        else max(latencies_with_idx)
    )
    ...
    print()
    print(
        f"{'Metric':<25} {'Without Indexes':<20} {'With Indexes':<20} {'Improvement'}"
    )
    print("-" * 70)
    print(
        f"{'Mean Latency':<25} {mean_no_idx:<20.2f} {mean_with_idx:<20.2f} {improvement_pct:+.1f}%"
    )
    print(
        f"{'P95 Latency':<25} {p95_no_idx:<20.2f} {p95_with_idx:<20.2f} {(p95_no_idx - p95_with_idx) / p95_no_idx * 100:+.1f}%"
    )
    print()
    ...
    if speedup > 1.1:
        ...
        print(
            "⚠️  Payload indexes add overhead (may indicate query planner already optimizing)"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark with/without payload indexes"
    )
    parser.add_argument(
        "--collection", type=str, default="products", help="Source collection"
    )
    parser.add_argument(
        "--m-value", type=int, default=1, help="HNSW m parameter (default: 1)"
    )
    parser.add_argument(
        "--iterations", type=int, default=50, help="Iterations per test"
    )
