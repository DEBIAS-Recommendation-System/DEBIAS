...
def embed_products(
    ...
    for i, product in enumerate(products, 1):
        try:
            # Display progress
            title_display = product["title"][:60] + "..." if len(product["title"]) > 60 else product["title"]
            print(f"\n[{i}/{len(products)}] {title_display}")
            print(f"   Brand: {product['brand']} | Category: {product['category']} | ${product['price']:.2f}")
            ...
            if download_product_image(product["image_url"], image_path):
                ...
                text_description = f"{product['title']} {product['brand']} {product['category']}"
...
def main():
    ...
    # Ask user for limit
    try:
        limit_input = input("How many products to embed? (press Enter for ALL): ").strip()
    ...
    if success > 0:
        ...
        print("   ")
        print("   # Image search")
        print("   search_products(query_image_url='https://...jpg', collection_name='products')")
    else:
