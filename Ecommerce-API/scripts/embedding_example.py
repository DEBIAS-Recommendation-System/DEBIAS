...
def example_basic_usage():
    ...
    results = qdrant_service.search(
        query_text="wireless audio device", limit=3, score_threshold=0.3
    )
    ...
    try:
        ...
        print(
            f"  - (Collection info unavailable due to client/server version mismatch)"
        )
...
def example_product_search():
    ...
    queries = ["headphones for music", "footwear for running", "waterproof speaker"]
    ...
    for query in queries:
        ...
        for i, result in enumerate(results, 1):
            print(
                f"  {i}. {result['payload']['product_name']} (Score: {result['score']:.4f})"
            )
